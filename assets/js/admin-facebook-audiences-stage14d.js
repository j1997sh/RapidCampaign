...
async function init(){
 ...
 function render(){
   ...
   audienceList.innerHTML=rows.length?`<div class="performance-table-wrap"><table class="performance-table"><thead><tr><th>Area</th><th>Audience file</th><th>Meta Custom Audience</th><th>Match key</th><th></th></tr></thead><tbody>${rows.map(x=>`<tr><td><strong>${esc(x.area)}</strong></td><td><code>${esc(x.filename||`${x.area_key}.csv`)}</code></td><td><strong>${esc(x.audience_name)}</strong><small>${esc(x.audience_id)}</small></td><td><code>${esc(x.area_key)}</code></td><td><div class="button-row"><button class="btn secondary small" data-edit-audience="${x.id}">Edit</button><button class="btn danger-outline small" data-delete-audience="${x.id}">Delete</button></div></td></tr>`).join('')}</tbody></table></div>`:'<div class="workspace-empty-state"><div><h3>No Facebook audience mappings yet</h3><p>Import the audience mapping CSV once and every campaign can reuse it.</p></div></div>';
   document.querySelectorAll('[data-edit-audience]').forEach(b=>b.onclick=()=>{
     const x=audiences.find(a=>a.id===b.dataset.editAudience);if(!x)return;
     editingAudience=x;audienceDialogTitle.textContent='Edit audience mapping';audienceArea.value=x.area||'';audienceFilename.value=x.filename||'';audienceId.value=x.audience_id||'';audienceName.value=x.audience_name||'';audienceEditDialog.showModal();
   });
 ...
 addAudience.onclick=()=>{editingAudience=null;audienceDialogTitle.textContent='Add audience mapping';audienceEditForm.reset();audienceEditDialog.showModal()};
 document.querySelectorAll('[data-close-audience]').forEach(x=>x.onclick=()=>audienceEditDialog.close());
 audienceEditForm.onsubmit=async e=>{
   e.preventDefault();
   const before=editingAudience,oldKey=before?.area_key||'';
   const row=normaliseRow({area:audienceArea.value,filename:audienceFilename.value,audience_id:audienceId.value,audience_name:audienceName.value});
   if(!row.area||!row.audience_id||!row.audience_name)return msg('Area, Meta audience ID and Meta audience name are required.',true);
   const r=await sb.rpc('org_admin_upsert_facebook_audiences',{p_org:orgId,p_rows:[row]});if(r.error)return msg(r.error.message,true);
   if(before&&oldKey&&oldKey!==row.area_key){const del=await sb.rpc('org_admin_delete_facebook_audience',{p_audience:before.id});if(del.error)return msg('New mapping saved, but the old mapping could not be removed: '+del.error.message,true)}
   editingAudience=null;audienceEditDialog.close();audienceEditForm.reset();msg('Audience mapping saved.');await load();
 };
 await load();
}
