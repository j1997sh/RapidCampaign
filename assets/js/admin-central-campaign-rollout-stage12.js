...
async function init(){
 ...
 function facebookRows(settings){
   ...
   return (data?.sites||[]).map(site=>{
     ...
     const link=site.domain?(/^https?:\/\//i.test(site.domain)?site.domain:`https://${site.domain}`):`${settings.baseUrl}/${site.slug}`;
     const tags=`utm_source=facebook&utm_medium=paid_social&utm_campaign=${encodeURIComponent(settings.campaign)}&utm_content=${encodeURIComponent(site.slug)}&rc_area=${encodeURIComponent(site.area)}`;
 ...
 function renderFacebookPreflight(){
   const s=facebookSettings(),rows=facebookRows(s),missingAudience=rows.filter(x=>!x.audienceRef).length,missingCreative=rows.filter(x=>!x.creative).length,total=rows.length,totalBudget=Number(s.budget||0)*(s.budgetLevel==='adset'?total:1),ready=rows.filter(x=>x.audienceRef&&x.creative&&x.row.Link).length;
   facebookAdsPreflight.innerHTML=`<div class="facebook-preflight-kpis"><div><span>Ads</span><strong>${total}</strong></div><div><span>Audiences matched</span><strong>${total-missingAudience}/${total}</strong></div><div><span>Creatives matched</span><strong>${total-missingCreative}/${total}</strong></div><div><span>Ready</span><strong>${ready}/${total}</strong></div></div>${missingAudience||missingCreative?`<div class="state-banner error">${missingAudience?`${missingAudience} audience${missingAudience===1?'':'s'} missing. `:''}${missingCreative?`${missingCreative} creative${missingCreative===1?'':'s'} missing. `:''}You can export a draft CSV, but unmatched rows need attention before Meta upload.</div>`:'<div class="state-banner success">All microsites have a matched audience and uploaded creative.</div>'}<div class="facebook-preflight-table"><table><thead><tr><th>Area</th><th>Ad set</th><th>Landing page</th><th>Audience</th><th>Creative</th></tr></thead><tbody>${rows.slice(0,12).map(x=>`<tr><td>${esc(x.site.area)}</td><td>${esc(x.row['Ad Set Name'])}</td><td><code>${esc(x.row.Link)}</code></td><td class="${x.audienceRef?'':'facebook-missing'}">${esc(x.audienceRef||x.expectedAudience+' — missing ID')}</td><td class="${x.creative?'':'facebook-missing'}"><code>${esc(x.creative?.filename||'Missing — expected '+x.row['Image File Name'])}</code></td></tr>`).join('')}</tbody></table>${rows.length>12?`<small>Showing 12 of ${rows.length} ads.</small>`:''}</div>`;
 }
 ...
 downloadAudienceNamingSheet.onclick=()=>{
   ...
   const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`${(settings.campaign||'campaign').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-audience-naming-sheet.csv`;a.click();URL.revokeObjectURL(a.href);
 };
 facebookAdsForm.onsubmit=e=>{e.preventDefault();const settings=facebookSettings();if(!settings.pageId)return msg('Facebook Page ID is required for Page post ads.',true);if(!settings.baseUrl||settings.baseUrl==='https:/')return msg('Add the landing page base URL before exporting.',true);const rows=facebookRows(settings);if(!rows.length)return msg('Import at least one local area before generating a Meta CSV.',true);const missing=rows.filter(x=>!x.audienceRef||!x.creative);if(missing.length&&!confirm(`${missing.length} of ${rows.length} rows are missing an audience or uploaded creative. Download a draft CSV anyway?`))return;const csv=[META_ADS_HEADERS.map(csvCell).join(','),...rows.map(x=>META_ADS_HEADERS.map(h=>csvCell(x.row[h])).join(','))].join('\r\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`${(settings.campaign||'facebook-campaign').toLowerCase().replace(/[^a-z0-9]+/g,'-')}-meta-bulk-upload.csv`;a.click();URL.revokeObjectURL(a.href);msg(`${rows.length} Facebook ad row${rows.length===1?'':'s'} generated using the uploaded Ads Manager v2.3 column structure.`);facebookAdsDialog.close();};
